...
#figure(
  image("fig1.svg", width: 50%),
  caption: [Configuration for a penguin trying to defecate towards his/her rear side.
...
Moreover, we found that papers with creative and attention-grabbing titles tend to attract more readers and
citations, which supports our hypothesis that the title alone has the highest impact on citation count (see this link: #link("https://example.com/")).
Our findings suggest that researchers should pay more attention to crafting effective titles that accurately
and creatively summarize the main message of their research, as it can have a significant impact on the success
and visibility of their work.
...
#figure(
  [
    #table(
      columns: 9,
      align: (x, y) => if x == 0 { left } else if y < 2 { center } else { right },
      toprule,
      table.header(
        [],
        table.cell(colspan: 2, [Adelie]),
        table.cell(colspan: 2, [Chinstrap]),
        table.cell(colspan: 2, [Gentoo]),
        [$ F(2, 339) $],
        [$ eta^2 $],
      ),

      [Measure], [M], [SD], [M], [SD], [M], [SD], [], [],
      midrule, [Body mass (kg)], [3.7], [0.46], [3.73], [0.38], [5.08], [0.5], [343.63\*\*\*],
      [0.67], [Flipper length (mm)], [189.95], [6.54], [195.82], [7.13], [217.19], [6.48], [594.8\*\*\*],
      [0.78], [Bill length (mm)], [38.79], [2.66], [48.83], [3.34], [47.5], [3.08], [410.6\*\*\*],
      [0.71], bottomrule,
    )
...
#lorem(80)#footnote[Here is a footnote.]

#lorem(100)#footnote[Here is another footnote.]

== Citations of catchy titles
#lorem(50)

#quote([#lorem(30)], attribution: [#cite(<jacso2005may>, form: "prose")], block: true)
...
= Results
#lorem(320)#footnote[Here is a long footnote. #lorem(30)]
